...
- **Knockout games are mapped per round, as their teams resolve — and `--apply` fixes BOTH `external_id` and `kickoff`.** A knockout game only reconciles once its team slots are filled (by the post-result recompute) *and* TheSportsDB has published the fixture. So this is a **recurring chore through the knockout stage**, one round at a time. R32 (M73–M88) was backfilled 2026-06-29; R16/QF/SF/3rd/Final (M89–M104) follow as each round resolves. Two things are wrong at first for each knockout game: a null `external_id` (live/reported paths skip it) and a **wrong `kickoff`** — FIFA shifts knockout kickoffs **for broadcast** after the draw, and a wrong kickoff breaks both the live-score window (`now ≥ kickoff`) and the per-match prediction lock. Procedure per round:
  ...
  2. `bin/xtask --env prod reconcile-events --apply` — writes, **in place** (`reconcile::apply_matches`): the `external_id`s on all matched games, and the **corrected `kickoff` on knockout games** (from the event's real `strTimestamp`; group times are left untouched). It prints each `# kickoff M#: old -> new`. Non-destructive, idempotent, **preserves the resolved `team_id` slots**. Repeat for `dev`. Re-run if broadcast times move again.
  3. Paste the new mappings **and corrected kickoffs** into `tournaments/fwc26.json` and commit, so a future re-import keeps them.

  **Do NOT use `bin/deploy <env> data` (re-import) for this mid-tournament** — it `put_tournament`s the fixture wholesale and would reset the resolved knockout `team_id` slots to `null` until the next recompute. `--apply` exists precisely to avoid that. (No api/spa redeploy is needed either: the resolver reads `external_id` + `kickoff` from the table per request.)
  - **Parser gotcha:** TheSportsDB `strTimestamp` is UTC but **offsetless** (`"2026-06-29T17:00:00"`). `event_kickoff` must parse it as a naive UTC datetime — RFC3339 rejects it and the old fallback silently used `dateEvent` midnight, which once wrote midnight kickoffs to prod before being corrected (`fix` commit 2026-06-29).

...
pub struct Pool {
    ...
    /// Cosmetic, unique-per-pool display label that prefixes the pool's invite
    /// links (e.g. `SOUTH7K` in `SOUTH7K-AD9XK3P7QT`). A bare prefix resolves to
    /// the pool's owner invite (the "pool link"). Generated by the application
    /// layer; the high-entropy key is the invite `code`, not this.
    pub prefix: String,
}

/// A reusable referral channel into a pool (the invite/referral table,
/// `DATA_MODEL.md`). `code` is the globally-unique high-entropy suffix; whoever
/// joins with it becomes "invited by" `invited_by` into `pool_id`. One row per
/// member per pool; the pool's canonical link is the row whose
/// `invited_by == pool.owner`, minted at pool creation. Reusable (multi-claim);
/// `revoked` is the off-switch (re-mint to rotate). The per-joiner realized edge
/// lives on `Player.referrer`, not here.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Invite {
    pub code: String,
    pub pool_id: String,
    pub invited_by: PlayerId,
